    ...
    <div class="container">
        ...
        <div class="movie-info" data-id="{{ $movie->movie_id }}">
            ...
            <div class="movie-details">
                ...
                <p class="center"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#FF5F00"
                ...
                <p>Đạo diễn: <i>{{ $movie->directors }}</i></p>
                <p>Diễn viên: <i>{{ $movie->actor }}</i></p>
                <h3 class="title">NỘI DUNG PHIM:</h3>
                <p> {{ $movie->description }}</p>
                <p style="color: #FF5F00"><strong>Trailer:</strong> <a href="{{ $movie->trailer_link }}"
                        target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="21" height="21"
        ...
        <div class="showtimes" style="text-align: center;">
            @if ($showTimesGroupedByDate)
                <h2 style="color: #FF5F00">LỊCH CHIẾU</h2>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                        ...
                        success: function(cinemas) {
                            ...
                            if (cinemas.length > 0) {
                                cinemas.forEach(cinema => {
                                    // <p class="addr" style="text-align: left;">${cinema.address}</p> margin-top: 1.2rem;
                                    let cinemaItem =
                                        `<li>
                                        <div class="cinema-item" data-cinema-id="${cinema.cinema_id}"
                                        >
                                            <p class="addr" style="text-align: left;"><b>${cinema.name}</b> - ${cinema.address}</p>
                                        ...
                                        <div class="show-time-list" >
                                            <ul style="list-style-type: none;">
                                                <li>
                                                    <div></div>
                                                        <ul style="list-style-type: none; align-items: center; display: flex; flex-wrap: wrap; gap: 1.2rem; ">`;
                                    cinema.cinema_rooms.forEach(room => {
                                        room.show_times.forEach(
                                            showtime => {
                                                ...
                                                cinemaItem +=
                                                    `<li class="showtime-link" data-id="${showtime.showtime_id}"  style="border: .5px solid; border-radius: .4rem; cursor: pointer; line-height: 100%; padding: .8rem 1.2rem; transition: all .3s;">
                                                    ${formattedTime}
                                                </li>`;
                                            });
                                    });
            ...
            function showCinemadt() {
                ...
                const showtimeLists = document.querySelectorAll('.show-time-list');

                cinemaItems.forEach((item, index) => {
                    item.addEventListener('click', function() {
                        // Loại bỏ lớp 'active' từ tất cả các phần tử
                        cinemaItems.forEach((otherItem, otherIndex) => {
                            otherItem.classList.remove('active');
                            otherItem.querySelector('svg').classList.remove('rotated');
                            showtimeLists[otherIndex].classList.remove('active');
                        });

                        // Thêm lớp 'active' vào phần tử hiện tại
                        item.classList.add('active');
                        item.querySelector('svg').classList.add('rotated');
                        showtimeLists[index].classList.add('active');
                    });
                });
            }
            ...
            function displayShowtimeDetails(data) {
                ...
                detailHtml+=`<div class="seat-legend" >
                        <div class="legend-item">
                            <div class="seat none">NAME</div> Ghế trống
                        </div>
                        <div class="legend-item">
                            <div class="seat sold">NAME</div> Ghế đã đặt
                        </div>
                </div>`
